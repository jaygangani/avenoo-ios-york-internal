...
class ViewController: UIViewController, WKNavigationDelegate {
    ...
    override var preferredStatusBarStyle : UIStatusBarStyle {
        ...
        // Status Bar Color set to light always
...
//        windowScene?.statusBarManager?.statusBarStyle?.rawValue?.
        
        let view = UIView(frame:
                            CGRect(x: 0.0, y: 0.0, width: getStatusBarWidth(), height: getStatusBarHeight())
               )
        view.backgroundColor = UIColor.init(red: 8/250, green: 52/250, blue: 67/250, alpha: 1)
    ...
    func initWebView() {
        Avenoo.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(Avenoo.webView);
        Avenoo.webView.uiDelegate = self;
        Avenoo.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)
        
        if #available(iOS 15.0, *), adaptiveUIStyle {
            themeObservation = Avenoo.webView.observe(\.underPageBackgroundColor) { [unowned self] webView, _ in
                currentWebViewTheme = Avenoo.webView.underPageBackgroundColor.isLight() ?? true ? .light : .dark
...
//        toolbarView.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin, .flexibleWidth]
    ...
    func getStatusBarWidth() -> CGFloat {
        let winScene = UIApplication.shared.connectedScenes.first
        let windowScene = winScene as! UIWindowScene
        var statusBarHeight = windowScene.statusBarManager?.statusBarFrame.width ?? 200
...
//        self.splashBkgView.frame = CGRect(x: splashBkgFrame.minX, y: statusBarHeight, width: splashBkgFrame.width, height: splashBkgFrame.height)
    ...
    func overrideUIStyle(toDefault: Bool = false) {
        //    FIXME: Commented theme change
        //        if #available(iOS 15.0, *), adaptiveUIStyle {
        //            if (((htmlIsLoaded && !Avenoo.webView.isHidden) || toDefault) && self.currentWebViewTheme != .unspecified) {
        //                UIApplication
        //                    .shared
        //                    .connectedScenes
    ...
    @objc func loadRootUrl() {
        ...
        Avenoo.webView.load(URLRequest(url: launchUrl))
    }
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                Avenoo.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(Avenoo.webView.estimatedProgress);
    ...
    deinit {
        Avenoo.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: Avenoo.webView)
        }
      ...
      if message.name == "IOSBridge", let messageBody = message.body as? String {
                  print(messageBody)
          
                let fcmToken = AppDelegate.FirebaseTokenSaver.FCMTOKEN

            let script = "var event = new CustomEvent('IOSEvents', { detail: { data: '\(String(describing:fcmToken))'}}); window.dispatchEvent(event);"
          Avenoo.webView?.evaluateJavaScript(script)
                    { (response, error) in
                                    if let error = error {
                                        print("title error")
                                        print(error)
